...
class UserInherit(models.Model):
    _inherit = 'res.users'
    send_notification = fields.Boolean('Send Notification to this user')
...
class HrEmployeeDocument(models.Model):
    ...
    state = fields.Selection([('active','Active'),('expiry','Expired')])
    ...
    def mail_reminder(self):
        ...
        for record in self.search([('expiry_date', '!=', False)]):
            ...
            if is_notification_day:
                employee_manager_name = record.employee_ref_id.coach_id.name
                employee_name = record.employee_ref_id.name
                document_name = record.name
                expiry_date_str = str(record.expiry_date)
                mail_content = (
                    f"Hello {employee_manager_name},<br>Document {document_name} for {employee_name},"
                    f"is going to expire on {expiry_date_str}. "
                    "Please track process to renew it before the expiry date."
                )
                ...
                for rec in self.env['res.users'].sudo().search([('send_notification','=',True)]):
                    # record.activity_schedule('mail.mail_activity_data_todo',
                    #                    user_id=record.employee_ref_id.user_id.id,
                    #                    note=subject)
                    record.activity_schedule('mail.mail_activity_data_todo',
                                       user_id=rec.id,
                                       note=mail_content)
